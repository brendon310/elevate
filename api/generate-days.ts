...
export interface UserContext {
  ...
  recentMoods?: number[];
  missedDays?: number;
  reason?: 'relapse' | 'missed_days' | 'struggling';
}

export function buildPersonalizationBlock(ctx: UserContext): string {
  const lines: string[] = [];
  if (ctx.reason === 'relapse') lines.push('- The user just relapsed. Day 1 MUST acknowledge this with compassion — zero shame. Frame as "restarting". Include a relapse-prevention action.');
  if (ctx.reason === 'missed_days' && ctx.missedDays) lines.push(`- The user missed ${ctx.missedDays} day(s). Ease back gently. Day 1 should be lower effort.`);
  if (ctx.reason === 'struggling') lines.push('- The user is struggling. Make tasks smaller, more achievable, more encouraging.');
  if (ctx.recentTriggers?.length) lines.push(`- Recent triggers: ${ctx.recentTriggers.join(', ')}. Address at least 2 of these.`);
  if (ctx.recentMoods?.length) {
    const avg = ctx.recentMoods.reduce((a, b) => a + b, 0) / ctx.recentMoods.length;
    lines.push(`- Recent mood avg: ${avg.toFixed(1)}/10.${avg < 4 ? ' Keep tone extra gentle.' : ''}`);
  }
  return lines.length ? `\nPERSONALIZATION:\n${lines.join('\n')}\n` : '';
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { slug, trackName, category, startingPoint, motivation, obstacle, fromDay, count, userContext } = req.body as {

      ${userContext ? buildPersonalizationBlock(userContext) : ""}    slug: string; trackName: string; category: string;
